...
int create_single_mr(struct pingpong_context *ctx, struct perftest_parameters *user_param, int qp_index)
{
	...
	#ifdef HAVE_CUDA
	if (user_param->use_cuda) {
		// CUdeviceptr d_A;		cuHostAlloc
		// int error;
		void *d_A;
		CUresult error;
		const size_t gpu_page_size = 4 * 1024;
		...
		printf("cuMemHostAlloc() of a %zd bytes GPU buffer\n",
		       ctx->buff_size);
		error = cuMemHostAlloc(&d_A, size, CU_MEMHOSTALLOC_PORTABLE);
		if (error != CUDA_SUCCESS) {
			printf("cuMemHostAlloc error=%d\n", error);
			return FAILURE;
		}
		//check --------------------------------------------
		strcpy((char*)d_A, "12345");
		printf("GPU buffer before tranfer: %s \n", d_A);
